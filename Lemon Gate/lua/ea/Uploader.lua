...
/*==============================================================================================
	...
	Purpose: Encodes scripts for sending.
==============================================================================================*/
local TableEncode = {}
local TableDecode = {}
...
for I = 1, string_len( Escape ) do
	local Char = Escape[I]
	TableEncode[Char] = true
end

for Byte = 1,255 do
	local HexByte = Hex[ (Byte - Byte % 16) / 16 + 1 ] .. Hex[ Byte % 16 + 1 ]
	TableDecode[HexByte] = string_char( Byte )
	if TableEncode[string_char( Byte )] then
		TableEncode[string_char( Byte )] = "%" .. HexByte
	else
		TableEncode[string_char( Byte )] = string_char( Byte )
	end
end

function E_A.Encode( String )
	return string_gsub( String, ".", TableEncode ) 
end; local Encode = E_A.Encode

function E_A.Decode( String )
	return string_gsub( String, "%%(..)", TableDecode ) 
end; local Decode = E_A.Decode
...
if SERVER then
	...
	function Uploader.Rec_Chunk( Lenth, Player )
		...
		if Upload.Chunks == Chunks then -- used to be ChunkID == Chunks
			...
			local Script = Decode( Table_Concat( Upload.Data, "" ) )
...
if CLIENT then
	...
	function Uploader.Send_Script( Entity, Script )
		...
		local Data = GetChunks( Encode( Script ), 65000 )
		local Chunks = #Data
...
if CLIENT then
	...
	function Downloader.Rec_Chunk( Lengh )
		...
		if Download.Chunks == Chunks then -- used to be ChunkID == Chunks
			local Script = Decode( Table_Concat( Download.Data, "" ) )
