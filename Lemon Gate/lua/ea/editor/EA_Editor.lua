...
function PANEL:Paint( w, h )
	...
	if !input_IsMouseDown(MOUSE_LEFT) and self.MouseDown then
...
function PANEL:DrawText( w, h )
	...
	surface_DrawRect(0, 0, self.LineNumberWidth + 5, self:GetTall())
	...
	for i, cell in ipairs( self.PaintRows[Row] ) do
		if offset < 0 then
			if cell[1]:len( ) > -offset then
				...
				offset = line:len( )
				draw_SimpleText(line .. " ", "Fixedsys", self.LineNumberWidth + 10, (Row - self.Scroll.x) * self.FontHeight, cell[2])
			else
				offset = offset + cell[1]:len()
			end
		else
			draw_SimpleText(cell[1] .. " ", "Fixedsys", offset * self.FontWidth + self.LineNumberWidth + 10, (Row - self.Scroll.x) * self.FontHeight, cell[2])
			offset = offset + cell[1]:len( )
		end
	end
end
