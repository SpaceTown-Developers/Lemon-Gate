...
EA:SetCost( EA_COST_CHEAP ) // TODO: Better cost!

/*
EA:RegisterFunction( "egpClear", "wl:", "", function( self, Value )
	local this = Value(self)
	if !CanUseEGP( self, this ) then return end 
	local Alias = { player = self.Player, entity = self.Entity, prf = 0 }
	EGP:DoAction( this, Alias, "ClearScreen" )
	Update( self, this )
end )

EA:RegisterFunction( "egpBox", "wl:nvv", "", function( self, ValueA, ValueB, ValueC, ValueD )
	local this = ValueA(self)
	local index = ValueB(self)
	local pos = ValueC(self)
	local size = ValueD(self)
	if !CanUseEGP( self, this ) then return end 
	local bool, obj = EGP:CreateObject( this, EGP.Objects.Names["Box"], { index = index, w = size[1], h = size[2], x = pos[1], y = pos[2] }, self.Player )
	if bool then 
		local Alias = { player = self.Player, entity = self.Entity, prf = 0 }
		EGP:DoAction( this, Alias, "SendObject", obj ) 
		Update( self, this ) 
	end
end )

EA:RegisterFunction( "egpBoxOutline", "wl:", "", function( self, ValueA, ValueB, ValueC, ValueD )
	local this = ValueA(self)
	local index = ValueB(self)
	local pos = ValueC(self)
	local size = ValueD(self)
	...
	local bool, obj = EGP:CreateObject( this, EGP.Objects.Names["BoxOutlined"], { index = index, w = size[1], h = size[2], x = pos[1], y = pos[2] }, self.Player )
	if bool then 
		local Alias = { player = self.Player, entity = self.Entity, prf = 0 }
		EGP:DoAction( this, Alias, "SendObject", obj ) 
		Update( self, this ) 
...
EA:RegisterFunction( "egpRoundedBox", "wl:", "", function( self, ValueA, ValueB, ValueC, ValueD )
	local this = ValueA(self)
	local index = ValueB(self)
	local pos = ValueC(self)
	local size = ValueD(self)
...
EA:RegisterFunction( "egpRadius", "wl:nn", "", function( self, ValueA, ValueB, ValueC ) 
	...
	if (bool) then
		local Alias = { player = self.Player, entity = self.Entity, prf = 0 }
		if (EGP:EditObject( v, { radius = radius } )) then EGP:DoAction( this, Alias, "SendObject", v ) Update(self,this) end
	end
end )
...
do 
    ...
    -- BoxOutline
    --------------------------------------------------------
    EA:RegisterFunction( "egpBoxOutline", "wl:nvv", "", function( self, ValueA, ValueB, ValueC, ValueD ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local pos, tValueC = ValueC( self )
        local size, tValueD = ValueD( self )
    ...
    -- RoundedBox
    --------------------------------------------------------
    EA:RegisterFunction( "egpRoundedBox", "wl:nvv", "", function( self, ValueA, ValueB, ValueC, ValueD ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local pos, tValueC = ValueC( self )
        local size, tValueD = ValueD( self )
    ...
    -- RoundedBoxOutline
    --------------------------------------------------------
    EA:RegisterFunction( "egpRoundedBoxOutline", "wl:nvv", "", function( self, ValueA, ValueB, ValueC, ValueD ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local pos, tValueC = ValueC( self )
        local size, tValueD = ValueD( self )
    ...
    -- Text
    --------------------------------------------------------
    EA:RegisterFunction( "egpText", "wl:nsv", "", function( self, ValueA, ValueB, ValueC, ValueD ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
    ...
    EA:RegisterFunction( "egpTextLayout", "wl:nsvv", "", function( self, ValueA, ValueB, ValueC, ValueD, ValueE ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local text, tValueC = ValueC( self )
        local pos, tValueD = ValueD( self )
    ...
    EA:RegisterFunction( "egpPoly", "wl:n", "", function( self, ValueA, ValueB ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
    ...
    EA:RegisterFunction( "egpPoly", "wl:nt", "", function( self, ValueA, ValueB, ValueC ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local args, tValueC = ValueC( self )
    ...
    EA:RegisterFunction( "egpPolyOutline", "wl:n", "", function( self, ValueA, ValueB ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
    ...
    EA:RegisterFunction( "egpPolyOutline", "wl:nt", "", function( self, ValueA, ValueB, ValueC ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local args, tValueC = ValueC( self )
    ...
    EA:RegisterFunction( "egpAddVertices", "wl:nt", "", function( self, ValueA, ValueB, ValueC ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local args, tValueC = ValueC( self )
    ...
    */
    ...
    -- Line
    --------------------------------------------------------
    EA:RegisterFunction( "egpLine", "wl:nvv", "", function( self, ValueA, ValueB, ValueC, ValueD ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
    ...
    -- Circle
    --------------------------------------------------------
    EA:RegisterFunction( "egpCircle", "wl:nvv", "", function( self, ValueA, ValueB, ValueC, ValueD ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local pos, tValueC = ValueC( self )
        local size, tValueD = ValueD( self )
    ...
    -- Circle Outline
    --------------------------------------------------------
    EA:RegisterFunction( "egpCircleOutline", "wl:nvv", "", function( self, ValueA, ValueB, ValueC, ValueD ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local pos, tValueC = ValueC( self )
        local size, tValueD = ValueD( self )
    ...
    -- Triangle
    --------------------------------------------------------
    EA:RegisterFunction( "egpTriangle", "wl:nvvv", "", function( self, ValueA, ValueB, ValueC, ValueD, ValueE ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
    ...
    -- Triangle Outline
    --------------------------------------------------------
    EA:RegisterFunction( "egpTriangleOutline", "wl:nvvv", "", function( self, ValueA, ValueB, ValueC, ValueD, ValueE ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local vert1, tValueC = ValueC( self )
        local vert2, tValueD = ValueD( self )
    ...
    -- Wedge
    --------------------------------------------------------
    EA:RegisterFunction( "egpWedge", "wl:nvv", "", function( self, ValueA, ValueB, ValueC, ValueD ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local pos, tValueC = ValueC( self )
        local size, tValueD = ValueD( self )
    ...
    -- Wedge Outline
    --------------------------------------------------------
    EA:RegisterFunction( "egpWedgeOutline", "wl:nvv", "", function( self, ValueA, ValueB, ValueC, ValueD ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local pos, tValueC = ValueC( self )
        local size, tValueD = ValueD( self )
    ...
    -- 3DHolder
    --------------------------------------------------------
    EA:RegisterFunction( "egp3DTracker", "wl:nv", "", function( self, ValueA, ValueB, ValueC ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local pos, tValueC = ValueC( self )
    ...
    EA:RegisterFunction( "egpPos", "wl:nv", "", function( self, ValueA, ValueB, ValueC ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
        local pos, tValueC = ValueC( self )
    ...
    -- Size
    ----------------------------
    EA:RegisterFunction( "egpSize", "wl:nv", "", function( self, ValueA, ValueB, ValueC ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
    ...
    EA:RegisterFunction( "egpAngle", "wl:nvvn", "", function( self, ValueA, ValueB, ValueC, ValueD, ValueE ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
    ...
    EA:SetCost(20)
    EA:RegisterFunction( "egpGlobalPos", "wl:n", "v", function( self, ValueA, ValueB ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
    ...
    /* TODO
    EA:RegisterFunction( "egpGlobalVertices", "wl:n", "t", function( self, ValueA, ValueB ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
    ...
    */
    ...
    EA:RegisterFunction( "egpVertices", "wl:n", "t", function( self, ValueA, ValueB ) 
        local this, tValueA = ValueA( self )
        local index, tValueB = ValueB( self )
    ...
    EA:RegisterFunction( "egpMaxObjects", "n", "n", function( self,  ) 

        return EGP.ConVars.MaxObjects:GetInt()
    end )

    EA:RegisterFunction( "egpMaxUmsgPerSecond", "n", "n", function( self,  ) 
    ...
    EA:RegisterFunction( "egpCanSendUmsg", "n", "n", function( self,  ) 
    ...
    EA:RegisterFunction( "egpClearQueue", "n", "n", function( self,  ) 
    ...
    EA:RegisterFunction( "egpQueue", "n", "n", function( self,  ) 
    ...
    EA:RegisterFunction( "egpQueueClk", "n", "n", function( self,  ) 
    ...
    EA:RegisterFunction( "egpQueueClk", "nwl", "n", function( self, ValueA ) 
    ...
    -- Returns 1 if the current execution was caused by the EGP queue system regarding the entity <screen> OR if the EGP queue system finished in the current execution
    EA:RegisterFunction( "egpQueueClk", "ne", "n", function( self, ValueA ) 
        local screen, tValueA = ValueA( self )
    ...
    EA:RegisterFunction( "egpQueueScreen", "n", "e", function( self,  ) 
    ...
    EA:RegisterFunction( "egpQueueScreenWirelink", "n", "wl", function( self,  ) 
    ...
    EA:RegisterFunction( "egpQueuePlayer", "n", "e", function( self,  ) 
    ...
    EA:RegisterFunction( "egpQueueClkPly", "ne", "n", function( self, ValueA ) 
        local ply, tValueA = ValueA( self )
