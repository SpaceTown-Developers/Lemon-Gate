...
Core:AddFunction( "abs", "n", "n", "local %A = value %1", "((%A >= 0) and %A or -%A)" )

Core:AddFunction( "ceil", "n", "n", "local %A = value %1", "(%A - %A % -1)" )

Core:AddFunction( "ceil", "n,n", "n", "local %A, %B = value %1, 10 ^ math.floor(value %2 + 0.5)", "(%A - ((%A * %B) % -1) / %B)" )

Core:AddFunction( "round", "n", "n", "local %A = value %1", "(%A - (%A + 0.5) % 1 + 0.5)" )

Core:AddFunction( "round", "n,n", "n", "local %A = 10 ^ math.floor(value %2 + 0.5)", "(math.floor(value %1 * %A + 0.5) / %A)" )

Core:AddFunction( "int", "n", "n", "local %A = value %1", "((%A >= 0) and %A - %A % 1 or %A - %A % -1)" )

Core:AddFunction( "frac", "n", "n", "local %A = value %1", "(%A >= 0 and %A % 1 or %A % -1)" )

Core:AddFunction( "clamp", "n,n,n", "n", "local %A, %B, %C = value %1, value %2, value %3", "(%A < %B and %B or (%A > %C and %C or %A))" )

Core:AddFunction( "inrange", "n,n,n", "n", "local %A, %B, %C = value %1, value %2, value %3", "((%A < %B or %A > %C) and 0 or 1)" )

Core:AddFunction( "sign", "n", "n", "local %A = value %1", "(%A > %Round and 1 or (%A < -%Round and -1 or 0)" )
...
Core:AddFunction( "random", "n", "n", "local %A = value %1", "(math.random( ) * %A)" )
Core:AddFunction( "random", "n", "n", "local %A, %B = value %1, value %2", "(%A + math.random( ) * (%B - %A))" )

Core:AddFunction( "random", "n", "n", "math.random( value %1 )" )
Core:AddFunction( "random", "n", "n", "local %A, %B = value %1, value %2", "math.random( math.min( %A, %B ), math.max( %A, %B ) )" )
